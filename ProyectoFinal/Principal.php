...
if (!isset($telefono)) {  // Si no está definido el teléfono (usuario no logueado)
    ...
    // Consulta SQL para obtener todos los registros de la tabla 'visitantes'
    $consulta_sql = "SELECT * FROM visitantes";  
    ...
    // Muestra una tabla con los registros de los visitantes
    ...
    if ($count > 0) {  // Si hay registros en la base de datos
        ...
        while ($row = mysqli_fetch_assoc($resultado)) {  
            echo "<tr>";
            echo "<td>" . htmlspecialchars($row['nombre_visita']) . "</td>";  // Muestra el nombre de visita
            echo "<td>" . htmlspecialchars($row['telefono']) . "</td>";  // Muestra el teléfono
            echo "<td>" . htmlspecialchars($row['letra_torre']) . "</td>";  // Muestra la letra de la torre
            echo "<td>" . htmlspecialchars($row['numero_depa']) . "</td>";  // Muestra el número de depa
