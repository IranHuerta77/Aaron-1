...
// Definir la consulta SQL para obtener todos los registros de la tabla 'visitantes'
$consulta_sql = "SELECT * FROM visitas";
...
if (mysqli_num_rows($resultado) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($resultado)) {
        ...
        echo "<td>" . htmlspecialchars($row['nombre_visita']) . "</td>";
        // Mostrar el teléfono del usuario
        echo "<td>" . htmlspecialchars($row['telefono']) . "</td>";
        // Mostrar la letra del edificio
        echo "<td>" . htmlspecialchars($row['letra_torre']) . "</td>"; 
        // Mostrar el número del departamento
        echo "<td>" . htmlspecialchars($row['numero_depa']) . "</td>"; 
